...
class SequenceAnalyticsService:
    ...
    def get_dashboard_metrics(sequence_id=None) -> dict:
        ...
        pending_drafts_count = drafts.filter(status=DraftStatus.DRAFT_PENDING).count()
        enrollment_approval_count = enrollments.filter(status=EnrollmentStatus.WAITING_APPROVAL).count()
        approval_count = max(pending_drafts_count, enrollment_approval_count)
